...
const ReviewSection = ({ officeId, canReview }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || (lang === 'id' ? 'Gagal mengirim ulasan.' : 'Failed to submit review.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      <h2 style={{ fontSize: '1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        {lang === 'id' ? 'Ulasan Pengguna' : 'User Reviews'} ({reviews.length})
      </h2>
      ...
      {user && user.role !== 'admin' && canReview && (
        <div className="card" style={{ padding: '2rem', marginBottom: '2rem', backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
          <h3 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', color: 'var(--color-text-main)' }}>{lang === 'id' ? 'Berikan Ulasan Anda' : 'Write a Review'}</h3>
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label className="form-label">{lang === 'id' ? 'Komentar' : 'Comment'}</label>
              <textarea
                className="form-control"
                rows="3"
                placeholder={lang === 'id' ? 'Bagikan pengalaman Anda menyewa ruangan ini...' : 'Share your experience renting this room...'}
            ...
            {success && <p style={{ color: 'var(--color-success)', fontSize: '0.9rem', marginBottom: '1rem' }}>{lang === 'id' ? 'Ulasan berhasil dikirim! Terima kasih.' : 'Review submitted successfully! Thank you.'}</p>}
 
            <button type="submit" className="btn btn-primary" disabled={submitting} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Send size={18} /> {submitting ? (lang === 'id' ? 'Mengirim...' : 'Submitting...') : (lang === 'id' ? 'Kirim Ulasan' : 'Submit Review')}
            </button>
            <p style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.75rem' }}>
              {lang === 'id' 
                ...
                : '*Only users who have completed a booking can write a review.'}
            </p>
          </form>
        </div>
      )}
      ...
      {/* List Reviews */}
      {loading ? (
        <p>{lang === 'id' ? 'Memuat ulasan...' : 'Loading reviews...'}</p>
      ) : reviews.length > 0 ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem', backgroundColor: 'var(--color-secondary)', borderRadius: 'var(--border-radius)' }}>
          <p style={{ color: 'var(--color-text-muted)' }}>{lang === 'id' ? 'Belum ada ulasan untuk ruangan ini. Jadilah yang pertama!' : 'No reviews for this room yet. Be the first!'}</p>
        </div>
      )}
    </div>
  );
