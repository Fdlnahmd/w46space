...
import { getRuangan, getPemesanan } from '../../services/mockData';
import { Building, BookOpen, Users, DollarSign } from 'lucide-react';
  ...
  useEffect(() => {
    Promise.all([getRuangan(), getPemesanan()]).then(([ruangan, pemesanan]) => {
      const pendapatan = pemesanan
        ...
        .reduce((sum, curr) => sum + curr.totalHarga, 0);
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-3">
        ...
        <StatCard title="Pendapatan (Estimasi)" value={`Rp ${stats.pendapatan.toLocaleString('id-ID')}`} icon={DollarSign} color="var(--color-success)" />
      </div>
